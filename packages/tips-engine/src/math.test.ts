...
describe("TIPS Engine: Mathematical Invariants", () => {
    ...
    const createMockBonds = (years: number[], coupon: number = 0.02): BondInfo[] => {
        return years.map(y => ({
            cusip: `BOND-${y}`,
            maturity: `${y}-04-15`,
            coupon: coupon,
    ...
    test("Coupon Drip: Income from year 2 reduces need in year 1", () => {
        const bonds = createMockBonds([2026, 2027], 0.10); // 10% coupon for easy math
        ...
        const result = buildLadder(bonds, targetIncome, 2026, 2027, new Date("2026-01-01"));
    ...
    test("Rounding: Always meets or exceeds target income", () => {
        ...
        const result = buildLadder(bonds, targetIncome, 2026, 2028, new Date("2026-01-01"));
    ...
    test("Zero Need: Handles years fully covered by coupons", () => {
        ...
        const bonds: BondInfo[] = [
            { cusip: "BOND-2029", maturity: "2029-04-15", coupon: 0.01, price: 100, baseCpi: 100 },
            { cusip: "BIG-2030", maturity: "2030-04-15", coupon: 0.50, price: 100, baseCpi: 100 } // 50% coupon!
        ];
        const targetIncome = 10000;
        const result = buildLadder(bonds, targetIncome, 2029, 2030, new Date("2029-01-01"));
    ...
    test("Market Reality: Handles 0% real yield (Price = 100)", () => {
        const bonds = createMockBonds([2026], 0.00); // 0% coupon, 0% yield
        const result = buildLadder(bonds, 10000, 2026, 2026, new Date("2026-01-01"));
    ...
    test("Market Reality: Negative Yields (Price > 100)", () => {
        const bonds: BondInfo[] = [
            { cusip: "NEG-2026", maturity: "2026-04-15", coupon: 0.00, price: 105, baseCpi: 100 }
        ];
        const result = buildLadder(bonds, 10000, 2026, 2026, new Date("2026-01-01"));
