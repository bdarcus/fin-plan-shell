...
describe("TIPS Engine: Rebalancing Logic", () => {
    ...
    test("Sandwich Rebalance: Sells bonds before and after a newly discovered middle maturity", () => {
        ...
        const bonds: BondInfo[] = [
            { cusip: "BOND-2026", maturity: "2026-04-15", coupon: 0.02, price: 100, baseCpi: 100, yield: 0.02 },
            { cusip: "NEW-2027",  maturity: "2027-04-15", coupon: 0.02, price: 100, baseCpi: 100, yield: 0.02 },
            { cusip: "BOND-2028", maturity: "2028-04-15", coupon: 0.02, price: 100, baseCpi: 100, yield: 0.02 }
        ];
        ...
        const result = calculateRebalance(bonds, currentHoldings, targetIncome, 2026, 2028);
        ...
        const sell26 = result.trades.find(t => t.cusip === "BOND-2026" && t.action === "SELL");
        const sell28 = result.trades.find(t => t.cusip === "BOND-2028" && t.action === "SELL");
        const buy27  = result.trades.find(t => t.cusip === "NEW-2027" && t.action === "BUY");
