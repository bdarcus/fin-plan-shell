	...
	<section class="bg-white p-8 md:p-12 rounded-3xl border border-slate-200 shadow-sm overflow-hidden relative">
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
			<div class="lg:col-span-5 space-y-6">
				<div class="inline-flex items-center px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-black uppercase tracking-widest">Retirement Summary</div>
				...
				<div class="pt-6 border-t border-slate-100 flex items-center space-x-6">
					<button onclick={exportAllData} class="flex items-center text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-emerald-600 transition-colors">
						<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="mr-2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" x2="12" y1="3" y2="15" /></svg> Save Data
					</button>
					<label class="flex items-center text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors cursor-pointer">
						<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="mr-2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" x2="12" y1="3" y2="15" /></svg> Load Data
				...
				<div class="space-y-8">
					<div class="h-16 flex rounded-2xl overflow-hidden shadow-inner bg-slate-50 p-1">
						{#each chartData as item}
							<div class="{item.color} h-full transition-all hover:brightness-110 relative group" style="width: {(item.val / (summary?.monthlyTotal || 1)) * 100}%">
								<div class="absolute -top-10 left-1/2 -translate-x-1/2 hidden group-hover:block bg-slate-900 text-white p-2 rounded text-[10px] whitespace-nowrap z-50 shadow-xl">{item.label}: {formatCurrency(item.val)}</div>
							</div>
						{/each}
					</div>
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
						{#each chartData as item}
							<div class="space-y-1">
								<div class="flex items-center text-[10px] font-black uppercase tracking-widest text-slate-400"><span class="w-2 h-2 {item.color} rounded-full mr-2"></span>{item.label}</div>
					...
					<div>
						<div class="flex items-center justify-between mb-4">
							<div class="flex items-center space-x-3">
								<div class="p-2 bg-slate-50 text-slate-600 rounded-lg"><Icon /></div>
								<h3 class="text-lg font-bold text-slate-900">{m.name}</h3>
							</div>
							<button onclick={() => toggleModule(m.id)} aria-label="Toggle {m.name}" class="w-10 h-6 rounded-full transition-colors relative {isEnabled ? "bg-emerald-500" : "bg-slate-200"}">
						...
						{#if isEnabled}<div class="mb-6 py-4 border-t border-slate-50"><Dashboard /></div>{/if}
					</div>
					{#if isEnabled}
						<button onclick={() => manageModule(m.id)} class="w-full py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition-colors font-bold text-xs uppercase tracking-widest">Manage Module</button>
					{:else}
						<div class="w-full py-3 text-center text-[10px] font-black uppercase tracking-widest text-slate-400 bg-slate-50 rounded-xl">Module Disabled</div>
					{/if}
				</div>
			{/each}
		</div>
