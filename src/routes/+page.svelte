		...
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
			<div class="lg:col-span-5 space-y-6">
				...
				<p class="text-slate-500 text-lg leading-relaxed max-w-md">
					...
					{#if summary?.horizon}
						Projection spans {Math.round(summary.horizon)} years.
					{/if}
				</p>

				<div class="pt-4 flex flex-wrap gap-4">
					{#if registry.getModule('smart-withdrawals')}
						<button
					...
					{#each registry.enabledModulesList.filter(m => m.id !== 'smart-withdrawals').slice(0, 1) as m}
						<button
							onclick={() => manageModule(m.id)}
							class="px-6 py-3 bg-white border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50 transition-colors"
							>Adjust {m.name}</button
						>
					{/each}
				</div>
						...
						{#each chartData as item}
							<div class="space-y-1">
								...
								<div class="text-[10px] text-slate-400 font-bold">
									{Math.round((item.val / (summary?.monthlyTotal || 1)) * 100)}% of total
								</div>
							</div>
						{/each}
					</div>
					...
					<div>
						<div class="flex items-center justify-between mb-4">
							<div class="flex items-center space-x-3">
								<div
									class="p-2 bg-slate-50 text-slate-600 rounded-lg group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-colors"
								>
									<Icon />
								</div>
							...
							<button
								...
								aria-label="Toggle {m.name} module"
						...
						<p class="text-xs text-slate-500 leading-relaxed mb-6 h-10 line-clamp-2">
							{m.description}
						</p>
