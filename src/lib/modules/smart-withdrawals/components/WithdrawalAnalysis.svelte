...
<div class="space-y-8">
	<header>
		...
		<p class="text-slate-500 mt-2">
			Dynamic spending simulation across 1,000 market scenarios.
		</p>
	</header>
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
			...
			<div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
				...
				<div class="font-serif text-3xl font-bold text-rose-600">
					{formatCurrency(calc.monteCarlo.p5[0] / 12)}
					<span class="text-xs font-sans text-slate-400">/mo</span>
				</div>
				<p class="text-[10px] text-slate-400 mt-2">
					Estimated floor in poor market scenarios.
				</p>
			</div>
		</div>
		...
		<div class="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
			<div class="flex justify-between items-center mb-12">
				...
				>
					...
					<div class="flex items-center">
						<span class="w-3 h-0.5 bg-emerald-600 rounded mr-2"></span> Median Projection
					</div>
				</div>
			</div>

			<div
				class="relative h-80 flex items-end gap-1 border-b border-slate-100 pb-2"
			>
				{#each calc.monteCarlo.years as year, i (year)}
					...
					>
						<div
							...
							style="bottom: {(calc.monteCarlo.p5[i] / maxVal) *
								100}%; height: {((calc.monteCarlo.p95[i] -
								calc.monteCarlo.p5[i]) /
								maxVal) *
								100}%"
						></div>
