...
<div class="space-y-8">
	...
	<header
		class="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-200 shadow-sm"
	>
		<div class="flex items-center space-x-4">
			...
			<div class="flex gap-2">
				{#each $ladderStore.ladders as ladder (ladder.id)}
					<button
						onclick={() => editLadder(ladder)}
						class="px-3 py-1.5 rounded-lg text-xs font-bold transition-all {activeLadderId ===
						ladder.id
	...
	{#if isAddingNew || activeLadderId}
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
			...
			>
				...
				>
					<h2 class="font-serif text-2xl font-bold text-slate-900">
						{activeLadderId ? "Edit" : "New"} Ladder
					</h2>
					<button
						onclick={() => {
							isAddingNew = false;
				...
				<div class="space-y-4">
					<div class="space-y-2">
						...
						<input
							...
							placeholder="e.g. Retirement Floor"
					...
					<div class="space-y-2">
						...
						>
							<option value="tips-manual"
								>TIPS Bond Calculator (Holdings-based)</option
							>
							<option value="simple-income"
								>Simple Income Projection (Target only)</option
							>
						</select>
					</div>
					...
					<div class="space-y-2">
						...
						>
							...
							<option value="tax-deferred"
								>Tax-Deferred (Traditional IRA/401k)</option
							>
						</select>
					</div>
					...
					<div class="space-y-2">
						<label
							for="income"
							class="block text-[10px] font-black uppercase tracking-wider text-slate-500"
							>Target Real Income ($/yr)</label
						>
						<input
							type="number"
							id="income"
							bind:value={income}
							class="w-full rounded-lg border-slate-200 focus:border-emerald-500 focus:ring-emerald-500 text-lg font-bold"
						/>
					</div>
				</div>
				...
				{#if error}
					<div
						class="p-4 bg-red-50 text-red-700 text-xs font-semibold rounded-lg border border-red-100"
					>
						⚠️ {error}
					</div>
				{/if}
			...
			<section class="lg:col-span-8 space-y-8">
				{#if currentType === "simple-income"}
					<div
						class="bg-white p-12 rounded-3xl border border-slate-200 shadow-sm text-center"
					>
						<div
						...
						<h3 class="font-serif text-2xl font-bold text-slate-900 mb-2">
							Income Projection Ready
						</h3>
						<p class="text-slate-500 max-w-sm mx-auto mb-8">
							This ladder will contribute a flat real income of <strong
								>${income.toLocaleString()}</strong
							>
							from <strong>{startYear}</strong> to <strong>{endYear}</strong> in the
							general planning dashboard.
						</p>
						<button
							onclick={saveSimple}
							class="px-8 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800"
							>Save to Plan</button
						>
					</div>
				{:else if !results}
					<div
						class="bg-white border-2 border-dashed border-slate-200 rounded-3xl p-16 text-center"
					>
						<div class="text-4xl mb-4">📜</div>
						<h3 class="font-serif text-2xl font-bold text-slate-900 mb-2">
							Configure your TIPS holdings.
						</h3>
						<p class="text-slate-500 max-w-sm mx-auto">
							Generate a concrete shopping list of bonds based on current market
							pricing.
						</p>
					</div>
				{:else}
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
						...
						>
							...
							<div class="font-serif text-3xl font-bold">
								{results.summary.rungCount} Years
							</div>
						</div>
						<div
							class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm"
					...
					>
						<div
							class="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center"
						>
							<h3 class="font-serif text-xl font-bold">
								Shopping List (Trade Ticket)
							</h3>
							<div class="flex gap-2">
								<button
									onclick={commitTips}
	...
	{:else}
		<div
			class="bg-white border border-slate-200 rounded-3xl p-12 text-center max-w-xl mx-auto shadow-sm"
		>
			<div
				class="w-16 h-16 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center mx-auto mb-6 text-2xl"
			>
				🏗️
			</div>
			<h3 class="font-serif text-2xl font-bold text-slate-900 mb-2">
			...
			<p class="text-slate-500 mb-8 text-sm">
				Build multiple bond ladders to cover different income phases or
				inflation-protection needs.
			</p>
			<button
				onclick={startAdding}
				class="px-8 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-500 shadow-lg transition-all"
				>Create Your First Ladder</button
			>
		</div>
	{/if}
</div>
