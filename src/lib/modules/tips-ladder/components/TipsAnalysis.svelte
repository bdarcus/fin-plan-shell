...
{#if state.ladders.length === 0}
	<div
		class="text-center py-20 bg-white rounded-3xl border border-slate-200 shadow-sm"
	>
		<div class="text-5xl mb-6">🔭</div>
		<h2 class="font-serif text-3xl font-bold text-slate-900 mb-4">
			No Ladders Found
		</h2>
		<p class="text-slate-500 max-w-sm mx-auto mb-8">
			You haven't designed or imported any bond ladders yet. Start by creating a
			new ladder in the configuration tab.
		</p>
	</div>
{:else}
	<div class="space-y-8">
		<header>
			<h1 class="font-serif text-4xl font-bold text-slate-900">
				Income Analysis
			</h1>
			<p class="text-slate-500 mt-2">
				Combined projection for all {state.ladders.length} active income sources.
			</p>
		</header>
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
			<div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
				...
				<div class="font-serif text-3xl font-bold text-slate-900">
					{formatCurrency(maxAnnualIncome)}
		...
		<div class="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
			<div class="flex justify-between items-center mb-8">
				<h3 class="font-serif text-xl font-bold">
					Aggregate Real Income Projection
				</h3>
			</div>
			...
			>
				{#each years as y (y.year)}
					{@const barHeight =
						maxAnnualIncome > 0 ? (y.income / maxAnnualIncome) * 100 : 0}
					<div
					...
					>
						...
						>
							<div
								class="absolute -top-8 left-1/2 -translate-x-1/2 hidden group-hover:block bg-slate-900 text-white p-2 rounded text-[10px] whitespace-nowrap z-50 shadow-xl"
							>
								{y.year}: {formatCurrency(y.income)}
							</div>
						</div>
						<div
							class="absolute -bottom-8 text-[9px] font-black text-slate-400 whitespace-nowrap overflow-hidden max-w-full"
						>
							{y.year % 5 === 0 ? y.year : ""}
						</div>
					</div>
		...
		>
			<table class="w-full text-left">
				...
				<tbody class="divide-y divide-slate-100">
					{#each state.ladders as ladder (ladder.id)}
						<tr class="hover:bg-slate-50 transition-colors">
							<td class="px-6 py-4">
								<div class="font-serif text-lg font-bold text-slate-900">
									{ladder.name}
								</div>
							</td>
							<td class="px-6 py-4">
								<div class="flex flex-col gap-1">
									<span
										class="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest w-fit {ladder.type ===
										'tips-manual'
											? 'bg-emerald-50 text-emerald-700'
											: 'bg-slate-50 text-slate-700'}"
									>
										{ladder.type === "tips-manual"
											? "TIPS Bonds"
											: "Simple Income"}
									</span>
									<span
										class="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest w-fit {ladder.taxStatus ===
										'tax-free'
							...
							<td
								class="px-6 py-4 text-right font-serif font-bold text-lg text-slate-900"
							>
