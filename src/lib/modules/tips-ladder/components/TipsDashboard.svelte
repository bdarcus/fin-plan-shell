<script lang="ts">
	...
	let ladderCount = $derived(ladderState.ladders.length);
	let totalIncome = $derived(
		ladderState.ladders.reduce((sum, l) => sum + l.annualIncome, 0),
	);
		...
		<div class="flex justify-between items-end">
			...
			<div class="text-right">
				...
				<div class="text-sm font-bold text-slate-700">
					{ladderCount}
					{ladderCount === 1 ? "Source" : "Sources"}
				</div>
			</div>
		</div>

		<div class="pt-3 border-t border-slate-50 space-y-2">
			{#each ladderState.ladders.slice(0, 3) as ladder (ladder.id)}
